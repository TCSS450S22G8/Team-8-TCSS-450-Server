...
router.post('/', (request, response, next) => {
    ...
    let getSaltQuery = "SELECT CREDENTIALS.SALTEDHASH, CREDENTIALS.SALT, CREDENTIALS.MEMBERID FROM MEMBERS " +
                   "INNER JOIN CREDENTIALS ON MEMBERS.MEMBERID = CREDENTIALS.MEMBERID " +
                   "WHERE UPPER(MEMBERS.EMAIL) = UPPER($1)"
        ...
        .then(result => {
            ...
            if (result.rowCount == 1) {
                ...
                if (salted_hash == result.rows[0].saltedhash) {
                    ...
                    let insertNewPassQuery = "UPDATE CREDENTIALS SET SALT = $1, SALTEDHASH = $2 WHERE CREDENTIALS.MEMBERID = $3"
